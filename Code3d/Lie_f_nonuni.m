...
x=-1:h:1; %Range of x(0,1)and specifying the grid points
y=-1:h:1;
z=-1:h:1;
x = transpose(x);
y = transpose(y);
...
normvector = sqrt(x_repeated3d.^2 + y_repeated3d.^2 + z_repeated3d.^2);

% C = log(abs(x_repeated)).*lap2d_nabla(nx,1);
Cx = ((2*(x_repeated3d))./(normvector.^2)+ x_repeated3d - x_repeated3d./normvector).*lap3d_ux_nonuni(ax,bx,nx,1);
Cy = ((2*(y_repeated3d))./(normvector.^2)+ y_repeated3d - y_repeated3d./normvector).*lap3d_uy_nonuni(ax,bx,nx,1);
Cz = ((2*(z_repeated3d))./(normvector.^2)+ z_repeated3d - z_repeated3d./normvector).*lap3d_uz_nonuni(ax,bx,nx,1);

Cx_adap = ((2*(x_repeated3d))./(normvector.^2)+ x_repeated3d - x_repeated3d./normvector - theta).*lap3d_ux_nonuni(ax,bx,nx,1);
Cy_adap = ((2*(y_repeated3d))./(normvector.^2)+ y_repeated3d - y_repeated3d./normvector - theta).*lap3d_uy_nonuni(ax,bx,nx,1);
Cz_adap = ((2*(z_repeated3d))./(normvector.^2)+ z_repeated3d - z_repeated3d./normvector - theta).*lap3d_uz_nonuni(ax,bx,nx,1);

Cx_ori = theta.*lap3d_ux_nonuni(ax,bx,nx,1);
Cy_ori = theta.*lap3d_uy_nonuni(ax,bx,nx,1);
Cz_ori = theta.*lap3d_uz_nonuni(ax,bx,nx,1);
...
[t_ref,u_ref_array] = ode45( @(t,y) myode(t,y,A,Cx,6,x_repeated3d,y_repeated3d,z_repeated3d,a,nx,theta,normvector,Cy,Cz) , [0 T], u0_ori, opts);
...
%         tStart = tic;
        [t_ref,u_adv_array] = ode45( @(t,y) myode(t,y,null_matrix,Cx,7,x_repeated3d,y_repeated3d,z_repeated3d,a,nx,theta,normvector,Cy,Cz), [0 k], u0, opts);
        u_adv = (u_adv_array(end,:)).';
%         tEnd = toc(tStart)
        %% Compute again the numerical solution of v equation in haft step
        [t_ref,u_dif_array] = ode45( @(t,y) myode(t,y,A,null_matrix,8,x_repeated3d,y_repeated3d,z_repeated3d,a,nx,theta,normvector,Cy,Cz), [0 k], u_adv, opts);
        u1 = (u_dif_array(end,:)).';
%         u1 = expm(k*A)*w1;
        u0 = u1;
...
for i = 1:5
    ...
    for j= 1:nt
        ...
        [t_ref,u_adv_array_mod] = ode45( @(t,y) myode(t,y,null_matrix,Cx_ori,9,x_repeated3d,y_repeated3d,z_repeated3d,a,nx,theta,normvector,Cy_ori,Cz_ori) , [0 k], u0, opts);
        u_adv_mod = (u_adv_array_mod(end,:)).';
        
        %% Compute again the numerical solution of v equation in haft step
        [t_ref,u_dif_array_mod] = ode45( @(t,y) myode(t,y,A,Cx_adap,10,x_repeated3d,y_repeated3d,z_repeated3d,a,nx,theta,normvector,Cy_adap,Cz_adap) , [0 k], u_adv_mod, opts);
