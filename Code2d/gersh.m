function [extreigs,Operations,fetch,store] = gersh(A,varargin)
  ...
  if issparse(A) && nargin==1
    ...
    Operations = 3*nnz(A);
    radius = sum (abs (AS), 1)';
    Operations = Operations + 2*nnz(AS);
    diag_AS = diag(AS);
    extreigs.SR = full (min ( diag_AS - (radius - abs (diag_AS))));
    extreigs.LR = full(max(diag_AS + (radius - abs(diag_AS))));
    Operations = Operations + 4*length(radius); 
    AS = (A - AS);
    diag_AS = diag(AS);
    Operations = Operations + nnz(A)+nnz(AS);
    radius = sum (abs(AS),1)';
    Operations = Operations + 2*nnz(AS);
