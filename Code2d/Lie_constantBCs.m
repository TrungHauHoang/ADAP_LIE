...
[t_ref,u_ref_array] = ode45( @(t,y) myode_time(t,y,A,C,1,X(:),Y(:),a,nx,theta) , [0 T], u0_ori, opts);
...
%         tStart = tic;
        [t_ref,u_adv_array] = ode45( @(t,y) myode_time(t,y,null_matrix,C,2,X(:),Y(:),a,nx,theta), [0 k], u0, opts);
        u_adv = (u_adv_array(end,:)).';
%         tEnd = toc(tStart)
        %% Compute again the numerical solution of v equation in haft step
        [t_ref,u_dif_array] = ode45( @(t,y) myode_time(t,y,A,null_matrix,3,X(:),Y(:),a,nx,theta), [0 k], u_adv, opts);
        u1 = (u_dif_array(end,:)).';
%         u1 = expm(k*A)*w1;
        u0 = u1;
...
for i = 1:5
    ...
    for j= 1:nt
        ...
        [t_ref,u_adv_array_mod] = ode45( @(t,y) myode_time(t,y,null_matrix,E,4,X(:),Y(:),a,nx,theta) , [0 k], u0, opts);
        u_adv_mod = (u_adv_array_mod(end,:)).';
        
        %% Compute again the numerical solution of v equation in haft step
        [t_ref,u_dif_array_mod] = ode45( @(t,y) myode_time(t,y,A,D,5,X(:),Y(:),a,nx,theta) , [0 k], u_adv_mod, opts);
...
loglog(vectork ,vectork - 0.75*(vectork-vec_err2_mod),'-.k');
xlabel('\tau');
ylabel('L_2 Error');
% title('Errors');
